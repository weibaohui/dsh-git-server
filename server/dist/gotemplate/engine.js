...
function lex(src) {
    ...
    while (true) {
        ...
        if (before)
            pieces.push({ kind: 'text', text: before });
        countNL(before);
        let i = start + 2;
        ...
        if (src[i] === '/' && src[i + 1] === '*') {
            ...
            if (endIdx < 0)
                throw new Error(`unterminated comment in template at line ${line}`);
            let j = endIdx + 2;
        ...
        if (closeIdx < 0)
            throw new Error(`unclosed action starting at line ${line}`);
        let end = closeIdx;
        let rightTrim = false;
        if (end > i && src[end - 1] === '-') {
            rightTrim = true;
...
function tokenize(src, line) {
    ...
    while (i < n) {
        ...
        if (c === '`') {
            ...
            if (end < 0)
                throw new Error(`line ${line}: unterminated raw string`);
        ...
        if (/[a-zA-Z_]/.test(c)) {
            ...
            if (mm) {
                const word = mm[0];
                if (word === 'true')
                    toks.push({ t: 'bool', v: true });
                else if (word === 'false')
                    toks.push({ t: 'bool', v: false });
                else if (word === 'nil')
                    toks.push({ t: 'nil' });
                else
...
function prevIsValueEnd(toks) {
    const last = toks[toks.length - 1];
    if (!last)
        return false;
    return (last.t === 'field' || last.t === 'dot' || last.t === 'var' || last.t === 'root' || last.t === 'number' ||
        last.t === 'string' || last.t === 'rawstring' || last.t === 'bool' ||
        (last.t === 'ident' && !BUILTINS.has(last.v)));
}
function readQuoted(src, start, line) {
    let i = start + 1;
    let out = '';
    while (i < src.length) {
        const c = src[i];
...
function readChar(src, start, line) {
...
class Parser {
    ...
    next() {
        const t = this.peek();
        if (!t)
            throw new Error(`line ${this.line}: unexpected end of action`);
        this.pos++;
        return t;
    }
    expectPunct(v) {
        const t = this.next();
        if (t.t !== 'punct' || t.v !== v)
            throw new Error(`line ${this.line}: expected ${v}, got ${JSON.stringify(t)}`);
    }
    parsePipeline() {
        ...
        if (this.peek()?.t === 'var') {
            ...
            if (this.peek()?.t === 'punct' && (this.peek().v === ':=' || this.peek().v === '=')) {
    ...
    parseCmd() {
        ...
        if (operands.length === 0)
            throw new Error(`line ${this.line}: empty command`);
...
function parseNodes(name, pieces, startIdx, p, defines, endTags) {
    const body = [];
    let idx = startIdx;
    const ctxCheck = (raw) => raw;
    void ctxCheck;
    while (idx < pieces.length) {
        const piece = pieces[idx++];
        if (piece.kind === 'text') {
            if (piece.text)
                body.push({ n: 'text', text: piece.text });
            continue;
        }
        ...
        switch (firstWord) {
            ...
            case 'define': {
                ...
                if (!m)
                    throw new Error(`line ${line}: bad define syntax`);
            ...
            case 'block': {
                ...
                if (!m)
                    throw new Error(`line ${line}: bad block syntax`);
            ...
            case 'template': {
                ...
                if (!m)
                    throw new Error(`line ${line}: bad template syntax`);
                const pipe = m[2].trim() ? p.parseAction(m[2].trim(), line) : null;
                body.push({ n: 'template', name: m[1], pipe });
                break;
            }
            ...
            default: {
                const node = parseActionNode(raw, piece, p, defines);
                if (node)
                    body.push(node);
            }
        }
    ...
    if (endTags.length)
        throw new Error(`template ${name}: unexpected EOF, expected ${endTags.join('/')}`);
    return { body, end: 'eof', next: idx };
}
function parseActionNode(raw, piece, p, _defines) {
    const pipe = p.parseActionTop(raw, piece.line);
    return { n: 'action', pipe };
}
export class TemplateError extends Error {
    tmplName;
    constructor(message, tmplName) {
...
function newScope(parent, root) {
    return { vars: new Map(), parent, root };
}
function lookupVar(s, name) {
    let cur = s;
    while (cur) {
        if (cur.vars.has(name))
            return { found: true, value: cur.vars.get(name) };
        cur = cur.parent;
    }
    return { found: false };
}
...
function execList(body, dot, scope, set, st, buf, tmplName) {
    for (const node of body) {
        if (st.breakFlag || st.continueFlag)
            return;
        execNode(node, dot, scope, set, st, buf, tmplName);
    }
}
function execNode(node, dot, scope, set, st, buf, tmplName) {
    switch (node.n) {
        case 'text': {
            buf.push(node.text);
        ...
        case 'if': {
            ...
            if (node.elseBody)
                execList(node.elseBody, dot, scope, set, st, buf, tmplName);
            break;
        }
        case 'with': {
            for (const br of node.branches) {
        ...
        case 'template': {
            ...
            if (!sub)
                throw new TemplateError(`template not found: ${node.name}`, tmplName);
...
function updateScriptContext(st, text) {
    ...
    while (i < lower.length) {
        ...
        if (open >= 0 && (close < 0 || open < close)) {
            ...
            if (tagEnd < 0) {
                ...
                if (!lower.slice(open, open + 200).includes('src='))
                    st.inScript++;
                return; // rest of chunk is inside the (possible) tag; stop scanning
            }
            if (!lower.slice(open, tagEnd).includes('src='))
                st.inScript++;
            i = tagEnd + 1;
            continue;
        }
...
function iterateCollection(col) {
    if (col == null)
        return [];
    ...
    if (typeof col === 'string') {
        return Array.from(col).map((ch, i) => [i, ch]);
    }
    if (Array.isArray(col))
        return col.map((v, i) => [i, v]);
    if (col instanceof Map)
        return Array.from(col.entries()).sort((a, b) => String(a[0]) < String(b[0]) ? -1 : 1);
    if (typeof col === 'object') {
        return Object.keys(col).sort().map((k) => [k, col[k]]);
    }
    return [];
}
// ---------------------------------------------------------------- evaluation
function evalPipeline(pipe, dot, scope, set, st, tmplName) {
    const cmds = pipe.cmds;
    let val;
...
function evalExpr(expr, dot, scope, set, st, tmplName, nested = false, isHead = false) {
    switch (expr.e) {
        case 'pipeline':
        ...
        case 'operand': {
            ...
            switch (t.t) {
                ...
                case 'var': {
                    ...
                    if (!r.found)
                        throw new TemplateError(`undefined variable: $${t.v}`, tmplName);
                    return r.value;
                }
                case 'number':
                    return t.v;
                ...
                case 'ident': {
                    ...
                    if (BUILTINS.has(t.v))
                        return makeBuiltin(t.v);
                    if (set.funcs[t.v])
                        return set.funcs[t.v];
...
function resolveFieldChain(base, path, invokeNiladic = true) {
    let cur = base;
...
function callFunc(fn, args, tmplName) {
    ...
    try {
        ...
        if (e instanceof TemplateError)
            throw e;
        throw new TemplateError(`${e.message}`, tmplName);
    }
}
function makeBuiltin(name) {
    switch (name) {
        ...
        case 'eq':
            return (...args) => {
                for (let i = 1; i < args.length; i++) {
                    if (looseEq(args[0], args[i]))
                        return true;
                }
                return false;
            };
        case 'ne':
            return (a, b) => !looseEq(a, b);
        case 'lt':
        ...
        case 'print':
            return (...args) => goSprint(args);
        case 'printf':
            return (format, ...args) => goSprintf(String(format ?? ''), args);
        case 'println':
            return (...args) => goSprint(args) + '\n';
        case 'html':
            return (v) => goEscape(String(printPlain(v)));
        case 'js':
            return (v) => JSON.stringify(v) ?? 'null';
        case 'urlquery':
            return (v) => goQueryEscape(printPlain(v));
        case 'call':
            return (fn, ...args) => (typeof fn === 'function' ? fn(...args) : fn);
    }
...
export function isTrue(v) {
    if (v == null || v === false)
        return false;
    if (typeof v === 'number')
        return v !== 0 && !Number.isNaN(v) ? true : false;
    ...
    if (v instanceof Map)
        return v.size > 0;
    if (typeof v === 'object')
        return Object.keys(v).length > 0 || Object.getPrototypeOf(v) !== Object.prototype;
    return true;
}
// ---------------------------------------------------------------- printing
function printPlain(v) {
    ...
    if (typeof v === 'boolean')
        return v ? 'true' : 'false';
    if (Array.isArray(v))
        return '[' + v.map((x) => printPlain(x)).join(' ') + ']';
    if (v instanceof Map) {
...
function printValue(v, st) {
    // html/template semantics: top-level nil renders as '' in HTML context, 'null' in <script>
    if (v == null)
        return st.inScript > 0 ? 'null' : '';
    if (v instanceof SafeHTML)
        return v.html;
    if (typeof v === 'string' || v instanceof String)
        return st.inScript > 0 ? jsEscape(String(v)) : goEscape(String(v));
    if (typeof v === 'function')
        return '';
    if (typeof v === 'number')
        return st.inScript > 0 ? String(v) : printPlain(v);
    if (typeof v === 'boolean')
        return String(v);
...
function goSprint(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (i > 0 && typeof args[i] !== 'string' && typeof args[i - 1] !== 'string')
            out += ' ';
        out += s;
    }
    return out;
}
export function goSprintf(format, args) {
    let argIdx = 0;
    let out = '';
    let i = 0;
...
function formatVerb(verb, flags, width, prec, arg) {
    let s;
    switch (verb) {
        case 's':
            s = arg == null ? '%!s(<nil>)' : printPlain(arg);
            if (prec !== '')
                s = s.slice(0, Number(prec));
            break;
        case 'd': {
