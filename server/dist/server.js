...
function serveFile(res, file, cacheForever = true) {
    try {
...
function serveStaticPrefix(req, res, pathname) {
    ...
    if (fs.existsSync(customDir)) {
        ...
        const safe = path.normalize(rel).replace(/^(\.\.[/\\])+/, '');
        if (safe && serveFile(res, path.join(customDir, safe)))
            return true;
    }
    return false;
}
/** Serve the SPA shell (public/dist/index.html) with WebContext substitution. */
function serveSPA(c, statusCode = 200) {
    ...
    if (pathname.startsWith('/assets/') || pathname.startsWith('/src/') || pathname.startsWith('/img/')) {
        const rel = pathname.replace(/^\/+/, '');
        const safe = path.normalize(rel).replace(/^(\.\.[/\\])+/, '');
        if (pathname.startsWith('/img/') && serveFile(c.res, path.join(conf.workDir, 'public', safe), false))
            return;
        if (serveFile(c.res, path.join(distDir, safe)))
            return;
        if (pathname.startsWith('/img/') && serveFile(c.res, path.join(distDir, safe), false))
            return;
    }
...
async function handleRequest(req, res) {
    ...
    if (handleGitHTTP(req, res, pathname))
        return;
    ...
    if (customAvatarMatch) {
        if (serveFile(res, path.join(conf.avatarUploadPath, customAvatarMatch[1]), false))
            return;
        res.statusCode = 302;
    ...
    if (repoAvatarMatch) {
        if (serveFile(res, path.join(conf.repositoryAvatarUploadPath, repoAvatarMatch[1]), false))
            return;
        res.statusCode = 302;
        res.setHeader('Location', conf.subpath + '/img/avatar_default.png');
        res.end();
...
`);
    ...
    if (serveStaticPrefix(req, res, pathname))
        return;
    ...
    if ((req.method === 'GET' || req.method === 'HEAD') && assetFile) {
        ...
        if (serveFile(res, path.join(conf.workDir, 'public', 'dist', 'assets', safe), false))
            return;
        res.statusCode = 404;
        res.end();
        return;
...
function downloadAvatar(url) {
    return new Promise((resolve, reject) => {
        ...
        const req2 = mod.get(url, { timeout: 5000 }, (r) => {
            if (r.statusCode !== 200) {
                r.resume();
...
async function runChain(handlers, c) {
