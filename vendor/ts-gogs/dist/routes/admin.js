...
export async function Dashboard(c) {
    ...
    const counter = {
        User: db.db().prepare('SELECT COUNT(*) AS c FROM user WHERE type = 0').get().c,
        Org: db.db().prepare('SELECT COUNT(*) AS c FROM user WHERE type = 1').get().c,
        PublicKey: db.db().prepare('SELECT COUNT(*) AS c FROM public_key').get().c,
        Repo: db.db().prepare('SELECT COUNT(*) AS c FROM repository').get().c,
        Watch: db.db().prepare('SELECT COUNT(*) AS c FROM watch').get().c,
        Star: db.db().prepare('SELECT COUNT(*) AS c FROM star').get().c,
        Action: db.db().prepare('SELECT COUNT(*) AS c FROM action').get().c,
        Access: db.db().prepare('SELECT COUNT(*) AS c FROM access').get().c,
        Issue: db.db().prepare('SELECT COUNT(*) AS c FROM issue').get().c,
        Comment: db.db().prepare('SELECT COUNT(*) AS c FROM comment').get().c,
        Oauth: 0,
        Follow: db.db().prepare('SELECT COUNT(*) AS c FROM follow').get().c,
        Mirror: db.db().prepare('SELECT COUNT(*) AS c FROM mirror').get().c,
        Release: db.db().prepare('SELECT COUNT(*) AS c FROM release').get().c,
        LoginSource: db.db().prepare('SELECT COUNT(*) AS c FROM login_source').get().c,
        Webhook: db.db().prepare('SELECT COUNT(*) AS c FROM webhook').get().c,
        Milestone: db.db().prepare('SELECT COUNT(*) AS c FROM milestone').get().c,
        Label: db.db().prepare('SELECT COUNT(*) AS c FROM label').get().c,
        HookTask: db.db().prepare('SELECT COUNT(*) AS c FROM hook_task').get().c,
        Team: db.db().prepare('SELECT COUNT(*) AS c FROM team').get().c,
        UpdateTask: 0,
        Attachment: db.db().prepare('SELECT COUNT(*) AS c FROM attachment').get().c,
    };
...
export async function Operation(c) {
    ...
    if (op === 'delete_repo_activities') {
        ...
        setImmediate(async () => {
            try {
                const mirrors = db.db().prepare('SELECT repo_id FROM mirror').all();
                const { syncMirror } = await import('../mirror.js');
                for (const m of mirrors) {
                    try {
                        const repo = db.getRepoByID(m.repo_id);
                        if (repo)
                            await syncMirror(m.repo_id, repo.owner_id);
                    }
                    catch (e) {
        ...
        const ids = db.db().prepare('SELECT id FROM repository').all();
        for (const { id } of ids) {
...
export async function SendTestMail(c) {
...
export async function Monitor(c) {
...
export async function Users(c) {
    c.Title('admin.users');
    c.Data['PageIsAdmin'] = true;
    c.Data['PageIsAdminUsers'] = true;
    const page = Math.max(1, c.QueryInt('page'));
    const size = 50;
    const total = db.db().prepare('SELECT COUNT(*) AS c FROM user WHERE type = 0').get().c;
    const rows = db.db().prepare('SELECT * FROM user WHERE type = 0 ORDER BY id LIMIT ? OFFSET ?').all(size, (page - 1) * size);
    c.Data['Users'] = rows.map((r) => new db.User(r));
    c.Data['Total'] = total;
    c.Data['Page'] = newPaginater(total, size, page, 5);
    c.Success('admin/user/list');
}
export async function NewUser(c) {
    c.Title('admin.users.new_account');
    c.Data['PageIsAdmin'] = true;
    c.Data['PageIsAdminUsers'] = true;
    c.Success('admin/user/new');
}
export async function NewUserPost(c) {
    const form = await c.form();
    try {
...
export async function EditUserPost(c) {
    const user = db.getUserByID(c.ParamsInt64(':userid'));
    if (!user) {
        c.NotFound();
        return;
...
export async function Organizations(c) {
    c.Title('admin.organizations');
    c.Data['PageIsAdmin'] = true;
    c.Data['PageIsAdminOrganizations'] = true;
    const page = Math.max(1, c.QueryInt('page'));
    const size = 50;
    const total = db.db().prepare('SELECT COUNT(*) AS c FROM user WHERE type = 1').get().c;
    const rows = db.db().prepare('SELECT * FROM user WHERE type = 1 ORDER BY id LIMIT ? OFFSET ?').all(size, (page - 1) * size);
    c.Data['Orgs'] = rows.map((r) => new db.User(r));
    c.Data['Total'] = total;
    c.Data['Page'] = newPaginater(total, size, page, 5);
    c.Success('admin/org/list');
}
export async function Repos(c) {
    c.Title('admin.repositories');
    c.Data['PageIsAdmin'] = true;
    c.Data['PageIsAdminRepositories'] = true;
    const page = Math.max(1, c.QueryInt('page'));
    const size = 50;
    const total = db.db().prepare('SELECT COUNT(*) AS c FROM repository').get().c;
    const rows = db.db().prepare('SELECT * FROM repository ORDER BY id LIMIT ? OFFSET ?').all(size, (page - 1) * size);
...
export async function DeleteRepo(c) {
    ...
    if (repo) {
        ...
        const issues = db.db().prepare('SELECT id FROM issue WHERE repo_id = ?').all(repoID);
...
export async function Notices(c) {
...
export async function DeleteNotices(c) {
    const form = await c.form();
...
export async function EmptyNotices(c) {
