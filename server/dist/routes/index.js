import { ignSignIn, reqSignIn, reqAdmin } from '../context.js';
...
export function registerWebRoutes(m) {
    m.get('/install', install.Install);
    m.post('/install', install.InstallPost);
    m.get('/', ignSignIn, home.Home);
    m.get('/explore', (c) => c.Redirect(conf.subpath + '/explore/repos'));
    ...
    for (const spa of ['/user/sign-in', '/user/sign-up', '/user/mfa', '/user/reset-password', '/user/activate']) {
        m.get(spa, (c) => { c.ServeWeb(); });
    }
    m.any('/user/activate_email', user.ActivateEmail);
    m.get('/user/email2user', user.Email2User);
    m.get('/user/avatar/:hash', (c) => c.Redirect(conf.subpath + '/img/avatar_default.png'));
    // ----- Admin -----
    ...
    m.get('/:username/:reponame/commit/:sha([a-f0-9]{7,40})', ignSignIn, (c) => { c.ServeWeb(); });
