...
function userDashboard(c) {
    ...
    const rows = db
        .db()
        .prepare(`SELECT * FROM action WHERE user_id = ? ORDER BY id DESC LIMIT 20`)
        .all(user0.id);
    ...
    const collab = db
        .db()
        .prepare(`SELECT r.* FROM repository r JOIN collaboration col ON col.repo_id = r.id
...
export function feedAction(row) {
...
export async function ExploreRepos(c) {
...
export async function ExploreUsers(c) {
    ...
    const total = db.db().prepare('SELECT COUNT(*) AS c FROM user WHERE type = 0 AND (lower_name LIKE ? OR lower(full_name) LIKE ?)').get(keyword, keyword).c;
    const rows = db
        .db()
        ...
        .all(keyword, keyword, size, (page - 1) * size);
...
export async function ExploreOrganizations(c) {
    ...
    const page = Math.max(1, c.QueryInt('page'));
    const size = conf.explorePagingNum;
    const total = db.db().prepare('SELECT COUNT(*) AS c FROM user WHERE type = 1 AND (lower_name LIKE ? OR lower(full_name) LIKE ?)').get(keyword, keyword).c;
    const rows = db
        .db()
...
export function newPaginater(total, pagingNum, current, numPages) {
    ...
    const pagesWindow = () => {
        ...
        for (let i = startNum; i <= endNum; i++)
            pages.push({ Num: i, IsCurrent: i === current });
        if (endNum < totalPages - 1)
            pages.push({ Num: -1 });
        else if (endNum < totalPages)
            pages.push({ Num: totalPages });
        return pages;
    };
    return {
        Total: total,
