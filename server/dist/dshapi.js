...
export function registerDshRoutes(m) {
    ...
    m.get('/api/dsh/repos/:o/:r/releases', async (c) => {
        ...
        if (!ar)
            return;
        const rows = db.db().prepare('SELECT r.id, r.tag_name, r.title, r.note, r.created_unix, u.name AS author FROM `release` r LEFT JOIN user u ON u.id = r.publisher_id WHERE r.repo_id = ? ORDER BY r.created_unix DESC').all(ar.repo.id);
        c.JSONSuccess(rows.map((r) => ({
    ...
    m.post('/api/dsh/repos/:o/:r/releases', async (c) => {
        ...
        try {
            ...
            const now = Math.floor(Date.now() / 1000);
            db.db().prepare('INSERT INTO `release` (repo_id, publisher_id, tag_name, lower_tag_name, title, note, is_draft, is_prerelease, created_unix) VALUES (?,?,?,?,?,?,0,0,?)').run(repo.id, user.id, tag, tag.toLowerCase(), title, note, now);
