...
function newAction(opts) {
    db.db()
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
        .run(opts.repo.owner_id, // feed receiver = repo owner (simplified; gogs also inserts for watchers)
    opts.opType, opts.doer.id, opts.doer.name, opts.repo.id, opts.repo.OwnerName(), opts.repo.name, opts.refName ?? '', opts.repo.is_private ?? 0, opts.content ?? '', nowUnix());
}
export function createRepoAction(doer, repo) {
    newAction({ opType: ActionType.CREATE_REPO, doer, repo });
}
export function forkRepoAction(doer, repo) {
    newAction({ opType: ActionType.FORK_REPO, doer, repo });
}
export async function commitRepoAction(doer, repo, refName, commits) {
...
export async function mirrorSyncPushAction(doer, repo, refName, commits) {
    const shortRef = refName.replace('refs/heads/', '');
    newAction({
...
export async function pushTagAction(doer, repo, tagName) {
...
export function deleteBranchAction(doer, repo, branch) {
...
export function createBranchAction(doer, repo, branch) {
...
export async function issueAction(opType, doer, repo, issue, content) {
    newAction({ opType, doer, repo, refName: '', content: content ?? String(issue.index) });
    const eventType = opType === ActionType.CREATE_ISSUE
        ? 'issues'
...
function issueActionLabel(opType) {
    switch (opType) {
...
function issuePayload(opType, doer, repo, issue) {
...
function pushPayload(doer, repo, refName, commits) {
    return {
        ref: refName,
        before: '',
        after: '',
...
function deliverHooks(repo, eventType, payload) {
    const hooks = db.db().prepare('SELECT * FROM webhook WHERE repo_id = ? AND is_active = 1').all(repo.id);
