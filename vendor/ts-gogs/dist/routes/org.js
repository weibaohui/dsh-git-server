...
export async function Teams(c) {
    ...
    c.Data['Teams'] = db.listTeamsByOrg(org.id).map((t) => ({ ...t, Organization: org }));
    c.Success('org/team/teams');
}
export async function TeamMembers(c) {
    const orgName = c.Params(':org');
    const org = db.getUserByUsername(orgName);
    ...
    c.Data['Org'] = org;
    c.Data['Team'] = team;
    c.Data['Title'] = team.name;
    c.Data['Members'] = db.listTeamMembers(team.id);
    c.Data['IsTeamMember'] = c.IsLogged && !!db.db().prepare('SELECT 1 FROM team_user WHERE team_id = ? AND uid = ?').get(team.id, c.UserID());
    c.Data['IsTeamAdmin'] = c.Data['IsTeamMember'] && team.authorize >= 3;
    c.Success('org/team/team');
}
export async function TeamRepositories(c) {
    const orgName = c.Params(':org');
    const org = db.getUserByUsername(orgName);
    const team = org ? db.getTeamByName(org.id, c.Params(':team')) : null;
    if (!org || !team) {
    ...
    c.Data['Team'] = team;
    c.Data['Title'] = team.name;
    c.Data['Repos'] = db.listTeamRepos(team.id);
    c.Success('org/team/repositories');
}
export async function NewTeam(c) {
...
export async function NewTeamPost(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    if (!org || org.type !== 1) {
        c.NotFound();
        return;
...
export async function EditTeam(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const team = org ? db.getTeamByName(org.id, c.Params(':team')) : null;
    if (!org || !team) {
        c.NotFound();
...
export async function EditTeamPost(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const team = org ? db.getTeamByName(org.id, c.Params(':team')) : null;
    if (!org || !team) {
        c.NotFound();
    ...
    const permission = String(form.permission ?? 'read');
    const authorize = permission === 'admin' ? 3 : permission === 'write' ? 2 : 1;
    db.db().prepare('UPDATE team SET description = ?, authorize = ? WHERE id = ?').run(String(form.description ?? ''), authorize, team.id);
    c.Redirect(conf.subpath + `/org/${org.name}/teams/${team.lower_name}`);
}
export async function DeleteTeam(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const team = org ? db.getTeamByName(org.id, c.Params(':team')) : null;
    if (!org || !team) {
        c.NotFound();
    ...
    if (team.lower_name === 'owners') {
        c.flash.Error(c.Tr('org.team.owners_cannot_delete'));
        c.Redirect(conf.subpath + `/org/${org.name}/teams`);
        return;
    }
    db.db().prepare('DELETE FROM team WHERE id = ?').run(team.id);
    db.db().prepare('DELETE FROM team_user WHERE team_id = ?').run(team.id);
    db.db().prepare('DELETE FROM team_repo WHERE team_id = ?').run(team.id);
    db.db().prepare('UPDATE user SET num_teams = num_teams - 1 WHERE id = ?').run(org.id);
    c.flash.Success(c.Tr('org.team.delete_success'));
    c.Redirect(conf.subpath + `/org/${org.name}/teams`);
}
export async function MembersAction(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    if (!org) {
        c.NotFound();
        return;
...
export async function TeamsAction(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const team = org ? db.getTeamByName(org.id, c.Params(':team')) : null;
    if (!org || !team) {
        c.NotFound();
    ...
    if (action === 'add') {
        if (!db.db().prepare('SELECT 1 FROM team_user WHERE team_id = ? AND uid = ?').get(team.id, user.id)) {
            db.db().prepare('INSERT INTO team_user (org_id, team_id, uid) VALUES (?,?,?)').run(org.id, team.id, user.id);
        ...
        db.db().prepare('DELETE FROM team_user WHERE team_id = ? AND uid = ?').run(team.id, user.id);
        db.db().prepare('UPDATE team SET num_members = MAX(num_members - 1, 0) WHERE id = ?').run(team.id);
    }
    c.Redirect(conf.subpath + `/org/${org.name}/teams/${team.lower_name}`);
}
export async function TeamsRepoAction(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const team = org ? db.getTeamByName(org.id, c.Params(':team')) : null;
    if (!org || !team) {
        c.NotFound();
    ...
    if (action === 'add') {
        if (!db.db().prepare('SELECT 1 FROM team_repo WHERE team_id = ? AND repo_id = ?').get(team.id, repo.id)) {
            db.db().prepare('INSERT INTO team_repo (org_id, team_id, repo_id) VALUES (?,?,?)').run(org.id, team.id, repo.id);
    ...
    c.Redirect(conf.subpath + `/org/${org.name}/teams/${team.lower_name}/repositories`);
}
...
export async function Invitation(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    if (!org) {
        c.NotFound();
        return;
...
export async function InvitationPost(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    if (!org) {
        c.NotFound();
        return;
...
export async function WebhooksEdit(c) {
    ...
    const hook = db.db().prepare('SELECT * FROM webhook WHERE id = ? AND org_id = ?').get(c.ParamsInt64(':id'), org.id);
    if (!hook) {
        c.NotFound();
        return;
    }
...
export async function OrgWebhooksNew(c) {
...
function orgHookBase(c, form, hookType, meta) {
    const org = requireOrg(c);
    if (!org) {
        c.NotFound();
        return;
    }
    const eventsObj = { push_only: false, send_everything: false, choose_events: false, events: {} };
    if (String(form.push_only ?? '') === 'on')
        eventsObj.push_only = true;
    if (String(form.send_everything ?? '') === 'on')
        eventsObj.send_everything = true;
    ...
    db.db()
       ...
       VALUES (0,?,?,?,?,?,0,?,?,?,0,?,?)`)
        .run(org.id, String(form.payload_url ?? ''), String(form.content_type ?? 'json') === 'form' ? 2 : 1, String(form.secret ?? ''), JSON.stringify(eventsObj), String(form.active ?? '') === 'on' ? 1 : 0, hookType, meta, now, now);
    c.flash.Success(c.Tr('repo.settings.add_hook_success'));
    c.Redirect(conf.subpath + `/org/${org.name}/settings/hooks`);
}
export async function OrgWebhooksNewPost(c) {
    const form = await c.form();
    orgHookBase(c, form, 1, '{}');
}
export async function OrgWebhooksSlackNewPost(c) {
    const form = await c.form();
...
export async function OrgWebhooksDiscordNewPost(c) {
    const form = await c.form();
    orgHookBase(c, form, 3, JSON.stringify({ username: String(form.username ?? '') }));
}
export async function OrgWebhooksDingtalkNewPost(c) {
    const form = await c.form();
    orgHookBase(c, form, 4, '{}');
}
export async function OrgWebhooksList(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    if (!org || org.type !== 1) {
        c.NotFound();
        return;
    ...
    c.Data['Org'] = org;
    c.Data['PageIsSettingsHooks'] = true;
    c.Data['Hooks'] = db.db().prepare('SELECT * FROM webhook WHERE org_id = ? ORDER BY id DESC').all(org.id).map((h) => db.goAlias({ ...h }));
    c.Success('repo/settings/webhook/list');
}
export async function OrgWebhooksEdit(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const hook = org ? db.db().prepare('SELECT * FROM webhook WHERE id = ? AND org_id = ?').get(c.ParamsInt64(':id'), org.id) : null;
    if (!org || !hook) {
        c.NotFound();
        return;
    }
...
export async function OrgWebhooksEditPost(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const hook = org ? db.db().prepare('SELECT * FROM webhook WHERE id = ? AND org_id = ?').get(c.ParamsInt64(':id'), org.id) : null;
    if (!org || !hook) {
        c.NotFound();
...
export async function OrgDeleteWebhook(c) {
    const org = db.getUserByUsername(c.Params(':org'));
    const hook = org ? db.db().prepare('SELECT id FROM webhook WHERE id = ? AND org_id = ?').get(Number((await c.form()).id ?? 0), org.id) : null;
    if (org && hook) {
