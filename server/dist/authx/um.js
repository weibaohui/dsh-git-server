...
let storeMod = null; // user-management store 模块（一次性加载）
let storeInst = null; // 当前 store 实例（users.json 指纹未变时复用）
let storeStamp = null;
let storeHome = null;
export function umStorePath() {
    return process.env.DSH_UM_STORE_PATH || '';
}
export function umServiceEnabled() {
    return !!umStorePath();
}
...
export function umAvailability() {
    if (!umServiceEnabled())
        return 'disabled';
    try {
 ...
 */
export async function umCheckLogin(username, password) {
    ...
    try {
        ...
        if (outcome.result === 'ok' && outcome.user) {
            if (outcome.user.totpEnabled)
                return { ok: false, reason: 'totp' };
...
export async function umFindUser(username) {
    const store = await getStore();
    if (!store || typeof store.findUserByUsername !== 'function')
        return null;
    try {
 ...
 */
export function ensureShadowUser(profile) {
    ...
    if (!user) {
        ...
        dbm.db().prepare('INSERT INTO user (name, lower_name, email, passwd, salt, type, is_admin, created_unix, updated_unix) VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?)').run(username, username.toLowerCase(), `${username.toLowerCase()}@users.dsh.local`, unusable, salt, isAdmin, now, now);
    }
    else {
        ...
        if (username !== bootstrapName) {
            ...
            args.push(createHash('sha256').update(randomBytes(32).toString('hex') + username).digest('hex'), randomBytes(16).toString('hex'));
        }
        if (patch.length) {
