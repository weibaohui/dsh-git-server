...
import { newUUID } from './db/db.js';
import { conf } from './conf.js';
export function deliverWebhook(hook, eventType, payload, repo) {
    ...
    const task = db
        .db()
        .prepare(`INSERT INTO hook_task (repo_id, hook_id, uuid, type, url, signature, payload_content, content_type, event_type, is_ssl, is_delivered, delivered, is_succeed)
       VALUES (?,?,?,?,?,?,?,?,?,?,0,?,0)`)
        .run(repo.id, hook.id, newUUID(), hookTaskType, hook.url, signature, payloadContent, hook.content_type ?? 1, eventType, hook.is_ssl ?? 0, process.hrtime.bigint() // delivered UnixNano (placeholder until completion)
    );
    ...
    const delivery = (async () => {
        ...
        try {
            const { status } = await postURL(String(hook.url), Buffer.from(body), isJSON ? 'application/json' : 'application/x-www-form-urlencoded', {
            ...
            db.db()
                .prepare(`UPDATE hook_task SET is_delivered = 1, delivered = ?, is_succeed = ?, request_content = ?, response_content = ? WHERE id = ?`)
                .run(delivered, status >= 200 && status < 300 ? 1 : 0, JSON.stringify({ headers: {} }), JSON.stringify({ status, headers: {}, body: '' }), taskID);
...
export const pendingDeliveries = new Set();
function slackText(eventType, payload) {
...
function postURL(url, body, contentType, headers) {
