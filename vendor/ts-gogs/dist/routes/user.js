...
function contextUser(c) {
    return c.ContextUser ?? c.Data['ContextUser'] ?? c.User;
}
export async function Profile(c) {
    ...
    if (tab === 'activity') {
        ...
        const rows = db
            .db()
            .prepare(`SELECT a.* FROM action a JOIN repository r ON r.id = a.repo_id
         WHERE a.act_user_id = ? AND (r.is_private = 0 OR r.owner_id = ?)
         ORDER BY a.id DESC LIMIT 20`)
            .all(u.id, c.UserID());
        c.Data['Feeds'] = rows.map(feedAction);
        for (const f of c.Data['Feeds'])
            db.goAlias(f);
...
export async function Followers(c) {
    const u = contextUser(c);
    ...
    const total = db.db().prepare('SELECT COUNT(*) AS c FROM follow WHERE follow_id = ?').get(u.id).c;
    c.Data['Total'] = total;
    c.Success('user/meta/followers');
}
export async function Following(c) {
    const u = contextUser(c);
    ...
    const total = db.db().prepare('SELECT COUNT(*) AS c FROM follow WHERE user_id = ?').get(u.id).c;
    c.Data['Total'] = total;
    c.Success('user/meta/followers');
}
...
export async function Dashboard(c) {
    ...
    const rows = db.db().prepare('SELECT * FROM action WHERE user_id = ? ORDER BY id DESC LIMIT 20').all(org ? org.id : c.UserID());
    c.Data['Feeds'] = rows.map(feedAction);
    for (const f of c.Data['Feeds'])
        db.goAlias(f);
    ...
    const collab = db.db().prepare('SELECT r.* FROM repository r JOIN collaboration col ON col.repo_id = r.id WHERE col.user_id = ?').all(c.UserID());
...
export async function Issues(c) {
    ...
    const repos = [
        ...
        ...db.db().prepare('SELECT r.* FROM repository r JOIN collaboration col ON col.repo_id = r.id WHERE col.user_id = ?').all(ctxUser.id),
    ].map((r) => {
        const repo = new db.Repository(r);
        repo.owner = db.getUserByID(repo.owner_id) ?? undefined;
        return repo;
    ...
    if (!isPullList) {
        repoScope = userRepoIDs.filter((id) => db.db().prepare('SELECT COUNT(*) AS c FROM issue WHERE repo_id = ? AND is_pull = 0').get(id).c > 0);
    }
    const scopeSQL = repoScope.length ? `i.repo_id IN (${repoScope.map(() => '?').join(',')})` : 'i.repo_id IN (-1)';
    // gogs GetUserIssueStats countSession
    const countWith = (closed, extraWhere, extraArgs, useScope) => {
        let w = 'i.is_closed = ? AND i.is_pull = ?';
        const a = [closed, isPullList ? 1 : 0];
        if (repoID > 0) {
            w += ' AND i.repo_id = ?';
            a.push(repoID);
        }
        ...
        return db.db().prepare(`SELECT COUNT(*) AS c FROM issue i WHERE ${w}`).get(...a).c;
    };
    const stats = {
    ...
    if (hasAnyRepo)
        stats.YourReposCount = countWith(0, '', [], true);
    ...
    const where = ['i.is_pull = ?', 'i.is_closed = ?'];
    const args = [isPullList ? 1 : 0, isShowClosed ? 1 : 0];
    if (repoID > 0) {
    ...
    const rows = db
        ...
        .all(...args, conf.issuePagingNum, (page - 1) * conf.issuePagingNum);
    const { labelView } = await import('./repo.js');
    const items = rows.map((r) => {
        let repo = repos.find((x) => x.id === r.repo_id);
        if (!repo) {
            const row = db.db().prepare('SELECT * FROM repository WHERE id = ?').get(r.repo_id);
        ...
        return db.goAlias({
            ...
            Labels: db.listIssueLabels(r.id).map((l) => db.goAlias({ ...labelView(l) })),
...
export async function Attachment(c) {
    const uuid = c.Params(':uuid');
    const row = db.db().prepare('SELECT * FROM attachment WHERE uuid = ?').get(uuid);
    if (!row) {
        c.NotFound();
        return;
    }
...
// ---------------------------------------------------------------- settings
async function settingsBase(c, title) {
    c.Title(title);
...
export async function SettingsEmails(c) {
    await settingsBase(c, 'settings.emails');
    c.Data['Emails'] = db.listEmailAddresses(c.UserID()).map((e) => db.goAlias({ ...e }));
    c.Success('user/settings/email');
}
export async function SettingsEmailPost(c) {
    await settingsBase(c, 'settings.emails');
    const form = await c.form();
...
export async function SettingsSSHKeys(c) {
    await settingsBase(c, 'settings.ssh_keys');
    c.Data['Keys'] = db.listPublicKeys(c.UserID()).map((k) => db.goAlias({ ...k }));
    c.Success('user/settings/sshkeys');
}
export async function SettingsSSHKeysPost(c) {
    await settingsBase(c, 'settings.ssh_keys');
    const form = await c.form();
...
export async function DeleteSSHKey(c) {
    const form = await c.form();
    const id = Number(form.id ?? 0);
    const key = db.getPublicKeyByID(id);
    if (key && key.owner_id === c.UserID()) {
...
export async function SettingsSecurity(c) {
...
export async function SettingsRepos(c) {
...
export async function SettingsLeaveRepo(c) {
    const form = await c.form();
...
export async function SettingsOrganizations(c) {
    await settingsBase(c, 'settings.organizations');
    c.Data['Orgs'] = db.listUserOrgs(c.UserID(), true);
    c.Success('user/settings/organizations');
}
export async function SettingsLeaveOrganization(c) {
    const form = await c.form();
...
export async function SettingsApplications(c) {
    await settingsBase(c, 'settings.applications');
    c.Data['Tokens'] = db.listAccessTokens(c.UserID()).map((t) => db.goAlias({ ...t }));
    c.Success('user/settings/applications');
}
export async function SettingsApplicationsPost(c) {
    await settingsBase(c, 'settings.applications');
    const form = await c.form();
    const name = String(form.name ?? '').trim();
    if (!name) {
    ...
    const exists = db.db().prepare('SELECT 1 FROM access_token WHERE uid = ? AND name = ?').get(c.UserID(), name);
    if (exists) {
...
export async function SettingsDeleteApplication(c) {
    const form = await c.form();
    const id = Number(form.id ?? 0);
...
export async function SettingsDelete(c) {
    await settingsBase(c, 'settings.delete');
    c.Data['Owner'] = c.User;
    c.Success('user/settings/delete');
}
export async function SettingsDeletePost(c) {
    const form = await c.form();
    ...
    const orgs = db.db().prepare('SELECT COUNT(*) AS c FROM org_user WHERE uid = ? AND is_owner = 1').get(c.UserID());
    if (owns > 0) {
 ...
 */
export async function ActivateEmail(c) {
    ...
    if (row && !row.is_activated) {
        db.db().prepare('UPDATE email_address SET is_activated = 1 WHERE id = ?').run(row.id);
    }
    c.flash.Success(c.Tr('settings.add_email_success'));
    c.RedirectSubpath('/user/settings/email');
}
/** GET /user/email2user — auto sign-in via verified email activation code. */
export async function Email2User(c) {
    const code = c.Query('code');
    const { verifyUserFromCode } = await import('../toolx.js');
    const parsed = verifyUserFromCode(code, (u) => db.getUserByUsername(u));
    if (!parsed || !parsed.valid) {
...
// ---------------------------------------------------------------- two-factor security
export async function SettingsTwoFactorEnable(c) {
    const twofactor = await import('../twofactor.js');
...
export async function SettingsTwoFactorEnablePost(c) {
    const twofactor = await import('../twofactor.js');
...
export async function SettingsTwoFactorRecoveryCodes(c) {
    const twofactor = await import('../twofactor.js');
    if (!twofactor.isTwoFactorEnabled(c.UserID())) {
        c.NotFound();
        return;
...
export async function SettingsTwoFactorRecoveryCodesPost(c) {
    const twofactor = await import('../twofactor.js');
    if (!twofactor.isTwoFactorEnabled(c.UserID())) {
        c.NotFound();
        return;
...
export async function SettingsTwoFactorDisable(c) {
    const twofactor = await import('../twofactor.js');
