...
export function handleGitHTTP(req, res, pathname) {
            ...
            if (action === 'git-upload-pack' || action === 'git-receive-pack') {
                ...
                if (svc === 'receive-pack') {
                    ...
                    const out = await git.statelessRPC(repoDir, svc, false, body, {
                        GOGS_AUTH_USER_ID: String(req.__authedUser?.id ?? 0),
                        GOGS_AUTH_USER_NAME: String(req.__authedUser?.name ?? ''),
                        GOGS_AUTH_USER_EMAIL: String(req.__authedUser?.email ?? ''),
