...
export class Conf {
    ...
    customConf;
    interpolate(value) {
        return String(value ?? '').replace(/%\((\w+)\)s/g, (_m, key) => {
    ...
    load(workDir, customDir, customConf) {
        ...
        this.raw = parseIni(defaultIni);
        // 2. custom overrides
        if (fs.existsSync(this.customConf)) {
            const custom = parseIni(fs.readFileSync(this.customConf, 'utf8'));
            for (const [sect, kv] of Object.entries(custom)) {
                ...
                if (!this.raw[sect] || typeof this.raw[sect] !== 'object')
                    this.raw[sect] = {};
        ...
        if (!ext)
            ext = `${this.protocol}://${this.domain}:${this.httpPort}/`;
        if (!ext.endsWith('/'))
            ext += '/';
        ...
        this.repositoryAvatarUploadPath = this.ensureAbs(this.get('picture', 'REPOSITORY_AVATAR_UPLOAD_PATH') ?? 'data/repo-avatars');
