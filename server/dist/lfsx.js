...
export function verifyLFSToken(token) {
    ...
    const p = { userID: Number(userID), repoID: Number(repoID), expiry: Number(expiry) };
    if (!Number.isFinite(p.expiry) || p.expiry < Date.now() / 1000)
        return null;
    return p;
}
function validOID(oid) {
...
async function authenticate(req) {
    ...
    if (user && verifyPassword(password, user.salt, user.passwd)) {
        if (is2FAEnabled(user.id))
            return null; // password auth disallowed for 2FA users
        return user;
    }
    ...
    for (const candidate of [username, password]) {
        ...
        if (row) {
            const u = db.getUserByID(row.uid);
            if (u)
                return u;
        }
    }
 ...
 */
export async function handleLFS(req, res, owner, repo, subPath) {
