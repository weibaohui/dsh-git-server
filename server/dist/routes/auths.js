...
function listSources() {
    return db.db().prepare('SELECT * FROM login_source ORDER BY id').all().map((s) => db.goAlias({ ...s }));
}
export async function Authentications(c) {
...
export async function NewAuthSource(c) {
...
export async function NewAuthSourcePost(c) {
    ...
    const cfg = { Provider: typeInfo(type).name.split(' ')[0] };
    if (type === 2) {
        Object.assign(cfg, {
...
export async function EditAuthSource(c) {
    const id = c.ParamsInt64(':authid');
    const row = db.db().prepare('SELECT * FROM login_source WHERE id = ?').get(id);
    if (!row) {
        c.NotFound();
        return;
    }
...
export async function EditAuthSourcePost(c) {
    const id = c.ParamsInt64(':authid');
    const row = db.db().prepare('SELECT * FROM login_source WHERE id = ?').get(id);
    if (!row) {
        c.NotFound();
...
export async function DeleteAuthSource(c) {
    const id = c.ParamsInt64(':authid');
    const used = db.db().prepare('SELECT COUNT(*) AS c FROM user WHERE login_source = ?').get(id);
    if (used.c > 0) {
