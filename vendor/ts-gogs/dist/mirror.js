...
export function mirrorAddress(repoDir) {
    try {
...
function intervalSeconds(repoID) {
    const row = db.db().prepare('SELECT interval FROM mirror WHERE repo_id = ?').get(repoID);
...
export function parseRemoteUpdateOutput(output) {
    const refs = [];
...
async function fillNewSha(repoDir, change) {
    if (change.newSha === '') {
...
export async function syncMirror(repoID, doerID) {
    const mirror = db.db().prepare('SELECT * FROM mirror WHERE repo_id = ?').get(repoID);
    const repo = db.getRepoByID(repoID);
    if (!mirror || !repo)
        return;
    const repoDir = repo.RepoPath();
    const remoteArgs = ['remote', 'update'];
    if (mirror.enable_prune)
        remoteArgs.push('--prune');
        ...
        try {
            ...
            if (newEmpty) {
                ...
                if (oldEmpty) {
                    ...
                    commits = [head, ...await Promise.all(shas.map((s) => git.catFileCommit(repoDir, s).catch(() => null)))];
...
async function fire(doer, repo, opType, refName, content) {
    db.db()
    ...
    if (opType === 9)
        await actionsMod.pushTagAction(doer, repo, refName);
    else if (opType === 21)
        await actionsMod.createBranchAction(doer, repo, refName);
    else if (opType === 18 || opType === 22)
        await actionsMod.deleteBranchAction(doer, repo, refName);
}
