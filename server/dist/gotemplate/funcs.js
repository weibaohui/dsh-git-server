...
export function shortSHA1(sha) {
    return String(sha).length > 10 ? String(sha).slice(0, 10) : String(sha);
}
export function ellipsis(str, threshold) {
    str = String(str ?? '');
    if (str.length <= threshold || threshold < 0)
        return str;
...
export function goFormatTime(d, layout) {
...
function timeSinceLabel(lang, key, ...args) {
    return i18n.translate(lang, `tool.${key}`, args);
}
function timeSinceText(then, lang) {
    const now = new Date();
    ...
    if (diff <= 0)
        return timeSinceLabel(lang, 'now');
    if (diff <= 2)
        return timeSinceLabel(lang, '1s', lbl);
    if (diff < MINUTE)
        return timeSinceLabel(lang, 'seconds', diff, lbl);
    if (diff < 2 * MINUTE)
        return timeSinceLabel(lang, '1m', lbl);
    if (diff < HOUR)
        return timeSinceLabel(lang, 'minutes', Math.floor(diff / MINUTE), lbl);
    if (diff < 2 * HOUR)
        return timeSinceLabel(lang, '1h', lbl);
    if (diff < DAY)
        return timeSinceLabel(lang, 'hours', Math.floor(diff / HOUR), lbl);
    if (diff < 2 * DAY)
        return timeSinceLabel(lang, '1d', lbl);
    if (diff < WEEK)
        return timeSinceLabel(lang, 'days', Math.floor(diff / DAY), lbl);
    if (diff < 2 * WEEK)
        return timeSinceLabel(lang, '1w', lbl);
    if (diff < MONTH)
        return timeSinceLabel(lang, 'weeks', Math.floor(diff / WEEK), lbl);
    if (diff < 2 * MONTH)
        return timeSinceLabel(lang, '1mon', lbl);
    if (diff < YEAR)
        return timeSinceLabel(lang, 'months', Math.floor(diff / MONTH), lbl);
    if (diff < 2 * YEAR)
        return timeSinceLabel(lang, '1y', lbl);
    return timeSinceLabel(lang, 'years', Math.floor(diff / YEAR), lbl);
}
function humanateBytes(s, base, sizes) {
    if (s < 10)
        return `${s} B`;
...
export function renderCommitMessage(full, msg, urlPrefix, metas) {
...
export function actionContent2Commits(act) {
    let out;
    try {
    ...
    out.AvatarLink = (email) => avatarLink(email);
    return out;
}
function inferSubmoduleURL(baseURL, mod) {
    ...
    if (scp)
        return `http://${scp[1]}/${scp[2].replace(/\.git$/, '')}`;
    if (url.startsWith('ssh://'))
        return url.replace('ssh://', 'http://');
    return url;
}
const MIME_IMAGE_EXT = new Set([
    '.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg', '.bmp', '.ico', '.tiff', '.avif',
]);
export function buildFuncMap() {
    return {
        ...
        LoadTimes: (start) => `${Date.now() - (start instanceof Date ? start.getTime() : Number(start))}ms`,
        AvatarLink: avatarLink,
        AppendAvatarSize: (url, size) => (String(url).includes('?') ? `${url}&s=${size}` : `${url}?s=${size}`),
        Safe: (raw) => new SafeHTML(String(raw ?? '')),
        Sanitize: (raw) => sanitizeHTML(String(raw ?? '')),
        Str2HTML: (raw) => new SafeHTML(sanitizeHTML(String(raw ?? ''))),
        NewLine2br: (raw) => String(raw ?? '').replaceAll('\n', '<br>'),
        TimeSince: (t, lang) => {
            const d = toTime(t);
            return new SafeHTML(`<span class="time-since" title="${htmlEscape(goFormatTime(d, conf.timeFormatLayout))}">${timeSinceText(d, lang ?? 'en-US')}</span>`);
        },
        RawTimeSince: (t, lang) => timeSinceText(toTime(t), lang ?? 'en-US'),
        FileSize: fileSize,
        Subtract: (a, b) => Number(a) - Number(b),
        Add: (a, b) => a + b,
        ActionIcon: actionIcon,
        DateFmtLong: (t) => goFormatTime(toTime(t), 'RFC1123Z'),
        DateFmtShort: (t) => goFormatTime(toTime(t), 'Jan 02, 2006'),
        ...
        DiffFileTypeToStr: (t) => ({ add: 'add', change: 'modify', delete: 'del', rename: 'rename' }[String(t)] ?? ''),
        DiffLineTypeToStr: (t) => (t === 'add' ? 'add' : t === 'del' ? 'del' : t === 'section' ? 'tag' : 'same'),
        ...
        FilenameIsImage: (filename) => MIME_IMAGE_EXT.has(path.extname(String(filename)).toLowerCase()),
        TabSizeClass: (_ec, _filename) => 'tab-size-8',
