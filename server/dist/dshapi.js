...
export function registerDshRoutes(m) {
    ...
    // ── 文件历史（git log -- path） ───────────────────────────────
    m.get('/api/dsh/repos/:o/:r/file-history', async (c) => {
        const ar = authRepo(c);
        if (!ar)
            return;
        const dir = ar.repo.RepoPath();
    ...
    // ── 删除文件（网页删文件提交） ──────────────────────────────────
    m.delete('/api/dsh/repos/:o/:r/files', async (c) => {
        const ar = authRepo(c, true);
        if (!ar)
            return;
        const body = await c.form();
    ...
    // ── 转移仓库所有权 / 清除 Wiki（危险区） ────────────────────────
    m.post('/api/dsh/repos/:o/:r/transfer', async (c) => {
        const ar = authRepo(c, true);
        if (!ar)
            return;
        if (ar.repo.OwnerName() !== ar.user.name && ar.user.is_admin !== 1) {
            c.JSON(403, { error: '仅仓库所有者可转移' });
            return;
        }
        const body = await c.form();
        const orgName = String(body.org ?? '');
        ...
        if (orgName && owner.type !== 1) {
            c.JSON(422, { error: '目标必须是组织' });
            return;
        }
        if (orgName) {
            const mem = db.db().prepare('SELECT 1 FROM org_user WHERE org_id = ? AND uid = ? AND is_owner = 1').get(owner.id, ar.user.id);
    ...
    m.post('/api/dsh/repos/:o/:r/wiki-wipe', async (c) => {
        const ar = authRepo(c, true);
        if (!ar)
            return;
    ...
    m.get('/api/dsh/admin/sysinfo', async (c) => {
        const header = String(c.req.headers.authorization ?? '');
        const token = /^token (.+)$/.exec(header)?.[1] ?? '';
        const user = token ? authenticateUserByToken(token) : null;
        ...
        let gitVersion = '';
        try {
            gitVersion = (await git.git(process.cwd(), '--version'))?.toString('utf8').trim() || '';
        }
        catch { }
        c.JSONSuccess({
    ...
    m.get('/api/dsh/admin/orgs', async (c) => {
        const header = String(c.req.headers.authorization ?? '');
        const token = /^token (.+)$/.exec(header)?.[1] ?? '';
        const user = token ? authenticateUserByToken(token) : null;
        if (!user || user.is_admin !== 1) {
        ...
        const rows = db.db().prepare("SELECT name, full_name, created_unix FROM user WHERE type = 1 ORDER BY id").all();
        c.JSONSuccess(rows.map((r) => ({ name: r.name, fullName: r.full_name || '', created: r.created_unix })));
    });
    m.post('/api/dsh/admin/ops', async (c) => {
        const header = String(c.req.headers.authorization ?? '');
        const token = /^token (.+)$/.exec(header)?.[1] ?? '';
        const user = token ? authenticateUserByToken(token) : null;
        if (!user || user.is_admin !== 1) {
        ...
        const repos = db.db().prepare('SELECT id FROM repository').all();
        if (op === 'sync_hooks') {
            for (const r of repos) {
                try {
                    const repo = db.getRepoByID(r.id);
                    if (repo)
                        svc.createDelegateHooks(repo.RepoPath());
                }
                catch { }
            }
        ...
        if (op === 'gc') {
            for (const r of repos) {
                try {
                    const repo = db.getRepoByID(r.id);
                    if (repo)
                        await git.git(repo.RepoPath(), 'gc', '--quiet');
                }
                catch { }
            }
            c.JSONSuccess({ ok: true, n: repos.length });
