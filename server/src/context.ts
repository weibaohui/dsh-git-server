...
function umRuntime(): { enabled: boolean; umCheck: (u: string, p: string) => { ok: boolean; reason?: string }; ensureAlignedUser: (u: string, p: string) => any | null } {
...
export function authenticateUserByBasic(header: string): { user: User; isBasic: boolean } | null {
  ...
  if (um.enabled) {
    ...
    if (check.ok) {
      ...
      if (aligned) return { user: aligned, isBasic: true };
    }
  }
