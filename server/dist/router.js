...
export class Router {
    ...
    head(pattern, ...h) { this.add('HEAD', pattern, ...h); }
    options(pattern, ...h) { this.add('OPTIONS', pattern, ...h); }
    any(pattern, ...h) { this.add('ANY', pattern, ...h); }
    /** Route(pattern, "GET,POST", handlers...) — macaron style */
    route(pattern, methods, ...h) {
        for (const m of methods.split(','))
            this.add(m.trim().toUpperCase(), pattern, ...h);
    }
    combo(pattern, ...h) {
    ...
    match(method, pathname) {
        ...
        for (const r of this.routes) {
            ...
            const hit = {
                route: r,
                params: r.paramNames.reduce((acc, name, i) => {
                    acc[name === '*' ? ':*' : ':' + name] = decodeURIComponent(m[i + 1] ?? '');
                    return acc;
                }, {}),
            };
            if (method === 'HEAD' && r.method === 'GET') {
                if (!getFallback || Router.moreSpecific(getFallback.route, hit.route) === hit.route)
                    getFallback = hit;
                continue;
            }
            if (!best || Router.moreSpecific(best.route, hit.route) === hit.route)
                best = hit;
        }
...
export class GroupBuilder {
    ...
    Group(prefix, fn, ...middleware) {
        const g = new GroupBuilder(this.router, this.prefix + prefix, [...this.middleware, ...middleware]);
        fn(g);
    }
    Get(pattern, ...h) { this.router.get(this.prefix + pattern, ...this.middleware, ...h); }
    Post(pattern, ...h) { this.router.post(this.prefix + pattern, ...this.middleware, ...h); }
    Put(pattern, ...h) { this.router.put(this.prefix + pattern, ...this.middleware, ...h); }
    Patch(pattern, ...h) { this.router.patch(this.prefix + pattern, ...this.middleware, ...h); }
    Delete(pattern, ...h) { this.router.delete(this.prefix + pattern, ...this.middleware, ...h); }
    Any(pattern, ...h) { this.router.any(this.prefix + pattern, ...this.middleware, ...h); }
    Route(pattern, methods, ...h) { this.router.route(this.prefix + pattern, methods, ...this.middleware, ...h); }
    Combo(pattern, ...h) { this.router.route(this.prefix + pattern, 'GET,POST', ...this.middleware, ...h); }
}
